...
- `MAIL_*`: credenciales de Brevo, el proveedor SMTP real que usa este proyecto (Mailpit no existe en este compose, solo en desarrollo) — servidor `smtp-relay.brevo.com`, puerto `587`, usuario tipo `xxxxx@smtp-brevo.com` y como contraseña tu SMTP key de Brevo. Pasos completos para crear el remitente y encontrar estas credenciales en `backend/.env.example`. Sin esto, los correos de verificación/reset de contraseña no salen. A diferencia de Render Free (ver `despliegue-produccion-servicios-gestionados.md`), un VPS propio no bloquea los puertos SMTP salientes, así que `BREVO_API_KEY` no hace falta aquí — puedes dejarla vacía.
