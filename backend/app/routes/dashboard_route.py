...
from app.schemas.dashboard_schema import (
    DashboardResumenResponse, TendenciaValor, ReservaProximaItem,
    ConteoNombre, ReservasPorMesItem,
)

router = APIRouter(prefix="/api/dashboard", tags=["Dashboard"])

DASHBOARD_CACHE_KEY = "dashboard:resumen"
...
def get_dashboard_resumen(db: Session = Depends(get_db), admin_id: int = Depends(require_admin)):
    ...
    conteo_estado = dict(
        db.query(Reserva.estado, func.count(Reserva.id_reserva))
        .group_by(Reserva.estado).all()
    )
    reservas_total = sum(conteo_estado.values())

    # ---------- Pagos por estado + ingresos reales ----------
    conteo_pago_estado = dict(
        db.query(Pago.estado, func.count(Pago.id_pago)).group_by(Pago.estado).all()
    )
    ...
    clientes_nuevos_mes = db.query(func.count(Cliente.id_cliente)).filter(
        Cliente.fecha_registro >= inicio_mes_actual
    ).scalar() or 0
    clientes_activos = db.query(func.count(func.distinct(Reserva.id_cliente))).scalar() or 0
    clientes_con_reserva_proxima = db.query(func.count(func.distinct(Reserva.id_cliente))).filter(
        Reserva.fecha_inicio >= hoy, Reserva.estado.in_(["confirmada", "pendiente"])
    ).scalar() or 0

    # ---------- Operación ----------
    checkins_proximos_7d = db.query(func.count(Reserva.id_reserva)).filter(
        Reserva.fecha_inicio.between(hoy, en_7_dias), Reserva.estado == "confirmada"
    ).scalar() or 0
    checkouts_proximos_7d = db.query(func.count(Reserva.id_reserva)).filter(
        Reserva.fecha_fin.between(hoy, en_7_dias), Reserva.estado == "confirmada"
    ).scalar() or 0
    solicitudes_cancelacion_pendientes = db.query(func.count(SolicitudCancelacion.id_solicitud)).filter(
    ...
    try:
        ...
        contactos_empresariales_pendientes = db.query(func.count(SolicitudCorporativa.id_solicitud)).filter(
            SolicitudCorporativa.estado == "nuevo"
        ).scalar() or 0
    except Exception:
        db.rollback()
        contactos_empresariales_pendientes = None
    ...
    reservas_mes_actual = db.query(func.count(Reserva.id_reserva)).filter(
        Reserva.fecha_reserva >= inicio_mes_actual
    ).scalar() or 0
    reservas_mes_anterior = db.query(func.count(Reserva.id_reserva)).filter(
        Reserva.fecha_reserva >= inicio_mes_anterior, Reserva.fecha_reserva < fin_mes_anterior
    ).scalar() or 0

    ingresos_mes_actual = float(db.query(func.coalesce(func.sum(Pago.monto), 0)).filter(
        Pago.estado == "pagado", Pago.fecha_pago >= inicio_mes_actual
    ).scalar() or 0)
    ingresos_mes_anterior = float(db.query(func.coalesce(func.sum(Pago.monto), 0)).filter(
        Pago.estado == "pagado", Pago.fecha_pago >= inicio_mes_anterior, Pago.fecha_pago < fin_mes_anterior
    ).scalar() or 0)
    ...
    cancelaciones_mes_actual = db.query(func.count(HistorialReserva.id_historial)).filter(
        HistorialReserva.estado_nuevo == "cancelada", HistorialReserva.fecha_cambio >= inicio_mes_actual
    ).scalar() or 0
    cancelaciones_mes_anterior = db.query(func.count(HistorialReserva.id_historial)).filter(
        HistorialReserva.estado_nuevo == "cancelada",
        HistorialReserva.fecha_cambio >= inicio_mes_anterior, HistorialReserva.fecha_cambio < fin_mes_anterior
    ).scalar() or 0

    clientes_nuevos_mes_anterior = db.query(func.count(Cliente.id_cliente)).filter(
        Cliente.fecha_registro >= inicio_mes_anterior, Cliente.fecha_registro < fin_mes_anterior
    ).scalar() or 0
    ...
    for (y, m) in meses_rango:
        ...
        total = db.query(func.count(Reserva.id_reserva)).filter(
            Reserva.fecha_reserva >= inicio, Reserva.fecha_reserva < fin
        ).scalar() or 0
        ...
        canceladas_mes = db.query(func.count(HistorialReserva.id_historial)).filter(
            HistorialReserva.estado_nuevo == "cancelada",
            HistorialReserva.fecha_cambio >= inicio, HistorialReserva.fecha_cambio < fin
        ).scalar() or 0
